...
TWOACTSOBSTACLE::TWOACTSOBSTACLE(std::vector<std::vector<double>> map,
...
TWOACTSOBSTACLE::TWOACTSOBSTACLE(std::vector<std::vector<double>> map,
    ...
    : nSubSegs(), subSegLengths(map), nDifficultyValues(3), nVelocityValues(2),
...
STATE* TWOACTSOBSTACLE::Copy(const STATE& state) const // Makes a copy of the state state
{
    const TWOACTSOBSTACLE_STATE& bmState = safe_cast<const TWOACTSOBSTACLE_STATE&>(state);
    TWOACTSOBSTACLE_STATE* newstate = MemoryPool.Allocate();
    *newstate = bmState;
    return newstate;
}

// NOT USED
void TWOACTSOBSTACLE::Validate(const STATE& state) const
{
    const TWOACTSOBSTACLE_STATE& bmState = safe_cast<const TWOACTSOBSTACLE_STATE&>(state);
}

// Used by standard planner
STATE* TWOACTSOBSTACLE::CreateStartState() const
{
    TWOACTSOBSTACLE_STATE* bmState = MemoryPool.Allocate();
    bmState->dps.clear(); 
    bmState->ps.clear();
    bmState->vs.clear();
    bmState->acs.clear();
...
/*
STATE* TWOACTSOBSTACLE::CreateStartStateFixedValues(std::vector<int> values) const
{
...
STATE* TWOACTSOBSTACLE::CreateStartState(std::vector<double*>* stateVarRelationships) const
{
    TWOACTSOBSTACLE_STATE* bmState = MemoryPool.Allocate();
    return bmState;
}

STATE* TWOACTSOBSTACLE::CreateStartState(vector<STATE*>* allParticles, vector<double> allParticleCumProbs) const
{
    TWOACTSOBSTACLE_STATE* bmState = MemoryPool.Allocate();
    bmState->dps.clear(); 
    bmState->ps.clear();
    ...
    partI--;
    const TWOACTSOBSTACLE_STATE& bmParticle = safe_cast<const TWOACTSOBSTACLE_STATE&>(*((*allParticles)[partI]));
...
void TWOACTSOBSTACLE::FreeState(STATE* state) const
{
    TWOACTSOBSTACLE_STATE* bmState = safe_cast<TWOACTSOBSTACLE_STATE*>(state);
    MemoryPool.Free(bmState);
}


bool TWOACTSOBSTACLE::Step(STATE& state, int action,
        int& observation, double& reward) const
{
    TWOACTSOBSTACLE_STATE& s = safe_cast<TWOACTSOBSTACLE_STATE&>(state);
    ...
    double dt, dt1, dt2;
    dt1 = s.v == 0 ? 3 : 1;
    dt1 *=  subSegLengths[s.curSegI][s.curSubsegJ];
...
bool TWOACTSOBSTACLE::LocalMove(STATE& state, const HISTORY& history,
...
void TWOACTSOBSTACLE::GenerateLegal(const STATE& state, const HISTORY& history,
...
void TWOACTSOBSTACLE::GeneratePreferred(const STATE& state, const HISTORY& history,
...
// Display methods -------------------------
void TWOACTSOBSTACLE::DisplayBeliefs(const BELIEF_STATE& beliefState,    // Displays all states in beliefState
        std::ostream& ostr) const
{
    cout << "TWOACTSOBSTACLE::DisplayBeliefs start" << endl;
    for (int i = 0; i < beliefState.GetNumSamples(); i++){
        const STATE* s = beliefState.GetSample(i);
        DisplayState(*s,cout);
    }
    cout << "TWOACTSOBSTACLE::DisplayBeliefs end" << endl;
}

void TWOACTSOBSTACLE::DisplayBeliefIds(const BELIEF_STATE& beliefState,    // Displays all states in beliefState
        std::ostream& ostr) const
{
    cout << "TWOACTSOBSTACLE::DisplayBeliefIds: [";
    for (int i = 0; i < beliefState.GetNumSamples(); i++){
        const STATE* s = beliefState.GetSample(i);
...
void TWOACTSOBSTACLE::DisplayBeliefDistribution(const BELIEF_STATE& beliefState,
        std::ostream& ostr) const
{
...
void TWOACTSOBSTACLE::DisplayState(const STATE& state, std::ostream& ostr) const
{
    const TWOACTSOBSTACLE_STATE& bmState = safe_cast<const TWOACTSOBSTACLE_STATE&>(state);
    ostr << endl;
...
void TWOACTSOBSTACLE::DisplayObservation(const STATE& state, int observation, std::ostream& ostr) const // Prints the observation
{
    switch (observation)
    {
...
void TWOACTSOBSTACLE::DisplayAction(int action, std::ostream& ostr) const // Prints the action performed
{
    if (action == 0)  
...
double TWOACTSOBSTACLE::JointProb(const STATE& state) const // Displays the STATE (grid with agent and rocks)
{
    // not implemented yet
    return 1.0;
}

void TWOACTSOBSTACLE::DisplayStateId(const STATE& state, std::ostream& ostr) const // Displays an id from rock values
{
    const TWOACTSOBSTACLE_STATE& bmState = safe_cast<const TWOACTSOBSTACLE_STATE&>(state);
    int id=0;
    string s="State id: ";
...
void TWOACTSOBSTACLE::DisplayStateHist(const STATE& state, const char* fileName) const // Displays an id from rock values
{
    const TWOACTSOBSTACLE_STATE& bmState = safe_cast<const TWOACTSOBSTACLE_STATE&>(state);
...
double TWOACTSOBSTACLE::ComputeParticleProbability(STATE& particle, std::vector<double*>* stateVarRelationships) const {
    TWOACTSOBSTACLE_STATE& oastate = safe_cast<TWOACTSOBSTACLE_STATE&>(particle);
    double probab=1;
